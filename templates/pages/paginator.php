<div>
    ...
    $pages = $page['pages'] ?? 1;
    $paginationUrl = "&action=$paginatorRedirect&pagesize=$pagesize&sortBy=" . ($params['sort']['sortBy'] ?? 'id') . "&order=" . ($params['sort']['order'] ?? 1);
    ?>
    <div class="d-flex flex-row flex-wrap justify-content-center justify-content-md-start">
        ...
        <?php endif; ?>
        <div class="col-4 col-md-2 col-xl-1 ms-3">
            <form action="/?action=<?php echo $paginatorRedirect; ?>" method="GET">
                <input type="hidden" name="action" value="<?php echo $paginatorRedirect; ?>" />
                <div class="input-group">
                <span class="input-group-text bg-white p-1 border-end-0"><i class="fas fa-bars"></i></span>
                    <select name="pagesize" id="pagesize" class="form-select form-select-sm border-start-0" onchange="this.form.submit()">
